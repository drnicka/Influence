...
function typeBadge(type) {
  ...
  if (t === 'theory') return { label: 'THEORY', cls: 'bg-voice-accent-soft text-voice-accent' }
  if (t === 'workblock') return { label: 'WORKBLOCK', cls: 'bg-voice-positive/10 text-voice-positive border border-voice-positive/20' }
  if (t === 'edit') return { label: 'EDIT', cls: 'bg-voice-muted/20 text-voice-text-secondary border border-voice-border' }
  if (t === 'design') return { label: 'DESIGN', cls: 'bg-voice-card/70 text-voice-text-secondary border border-voice-border' }
  if (t === 'decision') return { label: 'DECISION', cls: 'bg-voice-positive/10 text-voice-positive border border-voice-positive/20' }
  if (t === 'spec') return { label: 'SPEC', cls: 'bg-voice-muted/20 text-voice-text-secondary border border-voice-border' }
  if (t === 'slide') return { label: 'SLIDE', cls: 'bg-voice-card/70 text-voice-text-secondary border border-voice-border' }
  return null
}

export default function QuestionCard({ item, index }) {
  const t = (item?.type || 'text').toLowerCase()
  if (t === 'statement') return <StatementCard item={item} />
  if (t === 'public_statement') return <PublicStatementCard item={item} />
  if (t === 'question') return <QuestionPromptCard item={item} />
  if (t === 'workblock') return <WorkblockCard item={item} />
  if (t === 'edit') return <EditCard item={item} />
  if (t === 'image') return <ImageCard item={item} />

  const badge = typeBadge(item.type)

  return (
    <div className="bg-voice-surface rounded-xl border border-voice-border overflow-hidden">
      {/* Submitter info — Figma: 45x45 avatar, name + date row */}
      <div className="px-[18px] py-3.5 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {badge && (
            <span className={`inline-flex mt-1 items-center px-2 py-0.5 rounded-md text-[10px] font-semibold tracking-wide ${badge.cls}`}>
              {badge.label}
            </span>
          )}
        </div>
      ...
      <div className="px-[18px] py-4">
        <h3 className="text-[15px] font-semibold text-voice-text leading-snug">
          {item.title}
        </h3>
          ...
          <div className="mt-3 text-[13px] text-voice-text-secondary leading-[1.7]">
            <ReactMarkdown
              components={{
                ...
                code: (props) => <code className="px-1 py-0.5 rounded bg-voice-card/60 border border-voice-border text-[12px]" {...props} />,
        ...
        {/* Semantic tag if set */}
        {item.semanticTag && (
          <span className="inline-block mt-3 px-2.5 py-1 bg-voice-accent-soft rounded-md text-[10px] text-voice-accent font-medium">
            {item.semanticTag}
          </span>
        )}
      </div>
